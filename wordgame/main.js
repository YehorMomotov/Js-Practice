...
words = [
  {
    ...
    imgs: [
      "https://www.syl.ru/misc/i/ai/378693/2444688.jpg",
  ...
  },
  {
    ...
    imgs: [
      "https://www.collinsdictionary.com/images/full/rifle_224386324.jpg",
  ...
  },
  {
    ...
    imgs: [
      "https://growfood.pro/blog/wp-content/uploads/2017/10/sport2610-1.jpg",
  ...
  },
  {
    ...
    imgs: [
      "https://www.evromed.org.ua/wp-content/uploads/2017/03/Medetsina-v-polshe.jpg",
...
const printTheWord = () => {
  ...
  for (let i = 0; i < alreadyUsedChars.length; i++) {
    for (let j = 0; j < words[currentWord].word.length; j++) {
      if (
        words[currentWord].word[j].toLowerCase() ===
        alreadyUsedChars[i].toLowerCase()
      ) {
        formOfTheWord =
          formOfTheWord.substring(0, j) +
          words[currentWord].word[j] +
          formOfTheWord.substring(j + 1);
      }
    }
  }
...
const checkTheInput = (event) => {
  ...
  words[currentWord].word[0] == words[currentWord].word[0].match(/[A-Z]/gi)
    ? (regexp = /[A-Z]/gi)
    : (regexp = /[А-Я]/gi);
  strInput.value = "";
  ...
  if (inputChar != inputChar.match(regexp) && inputChar !== "") {
    event.target.value = "";
    errors.textContent =
      "Вводить можно только буквы, соответствующие раскладке вопроса!";
...
const printAttempts = () => {
  attempts.textContent = Math.ceil(
    words[currentWord].word.length +
      words[currentWord].word.length / 2 -
      alreadyUsedChars.length
  );
};
...
const printAlreadyUsed = () => {
  ...
  if (strInput.value == "") {
    for (let i = 0; i < alreadyUsedChars.length; i++) {
      formAlreadyUsed =
        alreadyUsedChars[i].toUpperCase() +
        alreadyUsedChars[i].toLowerCase() +
        ", ";
    }
  } else {
...
const checkUserDecisions = () => {
  if (
    theWord.textContent.toLowerCase() ==
      words[currentWord].word.toLowerCase() ||
    strInput.value.toLowerCase() == words[currentWord].word.toLowerCase()
  ) {
    defineScore();
    gameOver(true);
  }
...
const defineScore = () => {
  let currentScore = 0;
  if (
    theWord.textContent.toLowerCase() == words[currentWord].word.toLowerCase()
  ) {
    currentScore = Math.ceil(
      words[currentWord].word.length +
        words[currentWord].word.length / 2 -
        alreadyUsedChars.length
    );
    ...
    currentScore =
      2 *
      (words[currentWord].word.length -
        theWord.textContent.replace(/[*]/g, "").length);
    scored += currentScore;
